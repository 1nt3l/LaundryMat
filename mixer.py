...
SERVICE_USERNAME = "yourrpcuser"
SERVICE_PASSWORD = "yourrpcpassword"
SERVICE_PORT_NUMBER = "8332"
...
while True:
    ...
    rpc = ServiceProxy("http://%s:%s@127.0.0.1:%s" %
                       (SERVICE_USERNAME, SERVICE_PASSWORD,
                        SERVICE_PORT_NUMBER))

    # Connection to database.
    db = MySQLdb.connect(host="localhost", db=DATABASE_NAME,
    ...
    # Get list of all unspent outputs. Unspent outputs are funds that have
    # been sent into the mixer's wallet but have not been paid out yet. We
    # keep track of this so we can lock a user's input money when paying out
    # that user, so that users are guaranteed to get different bitcoins than
    # the ones they put in.
    # Min confirmations is set to 0 because we want to lock all payments
    # a particular user has put in, even unconfirmed ones.
    ...
    for account_row in account_list.fetchall():
        ...
        for current_input in inputs:
            ...
            if (current_input_address == current_input['address'] and
                ...
                # payments for.
                current_amount_due = (current_input_amount -
                ...
                if (current_amount_due >= current_minimum_payment):
                    ...
                    for output_address_row in output_addresses.fetchall():
                        ...
                        if (current_address_number > 1):
                            ...
                            min_delay = current_required_confirmations * 600
        ...
        for payment_row in payments_to_execute.fetchall():
            ...
            if (current_payment_datetime <= datetime.today()):
                ...
                for unspent_output in unspent_outputs:
                    ...
                    # If this output is for the current user's address...
                    if (current_input_address == unspent_output_address):
                        ...
                        unspent_output_ids = (unspent_output_ids +
                                                current_unspent_id)
                # Now that we've found all the unspent outputs for this
                # address, lock them.
